...
function callAddRoute(){
    wait.value=true
    let query=`
    mutation{
  AddRoute(destinationIndex: ${props.newDestIndex},
...
.then(function(data){
    if (data.errors) {
    addRouteResponse.value=data.error
    wait.value=false
  }
    else if(data.data.AddRoute.message){
        addRouteResponse.value=data.data.AddRoute.message
        wait.value=false
...
<template>
    <div class="grpcResponse">
        ...
        <pre v-if="!wait">{{ JSON.stringify(addRouteResponse, null, 2) }}</pre>
